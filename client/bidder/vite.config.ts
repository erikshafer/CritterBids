/// <reference types="vitest/config" />
import { fileURLToPath, URL } from "node:url";
import { defineConfig } from "vite";
import react from "@vitejs/plugin-react";
import tailwindcss from "@tailwindcss/vite";
import { VitePWA } from "vite-plugin-pwa";

// ADR 025 — build + dev integration.
//   base "/"            : the bidder app is served at the host root in production.
//   plugins             : React + the Tailwind v4-native Vite plugin (ADR 013) + the PWA plugin.
//   server.proxy        : forward /api and /hub to the API host so the browser stays
//                         same-origin in dev — no CORS, no API-host change. ws:true carries
//                         the SignalR negotiate POST + WebSocket upgrade for /hub/bidding.
// CRITTERBIDS_API_URL overrides the target if the API host runs on a non-default port.
const API_HOST = process.env.CRITTERBIDS_API_URL ?? "http://localhost:5180";

export default defineConfig({
  base: "/",
  plugins: [
    react(),
    tailwindcss(),
    // PWA "from day one" (ADR 013). M8-S2 wires the manifest + service-worker registration on the
    // first real shell (the S1 connection proof had no shell to install). App-shell precache only —
    // offline *data* scope (caching listings, queueing bids) stays an ADR-013-deferred question.
    VitePWA({
      registerType: "autoUpdate",
      injectRegister: "auto",
      manifest: {
        name: "CritterBids",
        short_name: "CritterBids",
        description: "CritterBids — live animal auctions. Browse the catalog and bid in real time.",
        theme_color: "#0f172a",
        background_color: "#0f172a",
        display: "standalone",
        start_url: "/",
        icons: [
          {
            src: "/critterbids.svg",
            sizes: "any",
            type: "image/svg+xml",
            purpose: "any maskable",
          },
        ],
      },
      workbox: {
        globPatterns: ["**/*.{js,css,html,svg,woff2}"],
      },
    }),
  ],
  resolve: {
    alias: {
      "@": fileURLToPath(new URL("./src", import.meta.url)),
    },
  },
  server: {
    port: 5173,
    proxy: {
      "/api": { target: API_HOST, changeOrigin: true },
      "/hub": { target: API_HOST, changeOrigin: true, ws: true },
    },
  },
  test: {
    environment: "jsdom",
    setupFiles: ["./src/test/setup.ts"],
    css: true,
  },
});
