...
- **[Narrative 003 - Bidder Starts Anonymous Session (Happy Path)](../narratives/003-bidder-starts-anonymous-session.md)** implements slice 0.2. Single-bidder perspective (BoldPenguin7, brought onstage from narrative 001's offstage cast); happy-path; companion to narrative 001 Moment 1 at finer grain (the Participants BC's mint cascade dramatised across three Moments: QR scan, system mint, catalog landing). Lived-code audit posture; two `code-update` findings surfaced (F001 lived-comment fix landed in this session's PR; F002 missing GET /api/participants/{id} endpoint routed to a stub follow-up at `docs/prompts/implementations/n003-fu-get-participant-endpoint.md`). Anchors BoldPenguin7's cross-narrative values: BidderId "Bidder 4523", credit ceiling $700.
- **[Narrative 004 - Seller Publishes and Withdraws Listing (Happy Path)](../narratives/004-seller-publishes-and-withdraws-listing.md)** implements slices 0.3, 1.1, 1.2. Single-seller perspective (GreyOwl12, brought onstage from narrative 001/002's offstage cast as the keyboard's seller); happy-path; first seller-perspective narrative for CritterBids. Mixed posture: lived M2 listing pipeline (Moments 1-4) plus forward-spec M4-S2 WithdrawListing (Moment 5). Three findings surfaced: F001 hardcoded FeePercentage placeholder (`document-as-intentional`), F002 missing SubmitListing HTTP endpoint (`code-update`, stub follow-up at `docs/prompts/implementations/n004-fu-submit-listing-endpoint.md`), F003 missing Approved intermediate state (`document-as-intentional`). Anchors the Vintage Folding Camera's listing-time fields as canonical: title "Vintage Folding Camera", Format Timed, StartingBid $40, ReservePrice null, BuyItNowPrice $80, ExtendedBiddingEnabled false, Duration 7 days, FeePercentage 0.10. Sibling-listing pattern (the camera) lets WithdrawListing dramatise without contradicting narrative 001's keyboard ground.
- **[Narrative 005 - Seller Watches Flash Auction Close (Happy Path)](../narratives/005-seller-watches-flash-auction-close.md)** implements slices 2.3, 3.1, 3.3, 5.1, 5.2. Single-seller perspective (GreyOwl12 continuing from narrative 004; observer-protagonist watching the keyboard go to auction); happy-path; first observer-protagonist narrative for CritterBids. Mixed posture: forward-spec Moment 1 for the M4-S5/S6 session-start cascade (no implementation prompts exist; spec source is W002 + narrative 001 Setting paragraph 2); lived M3 + M4-S1 Moments 2-4 (DCB place-bid cascade with reserve cross at $55, extended-bidding trigger and saga reschedule, gavel-fall with `BiddingClosed` + `ListingSold` cross-BC handoff to narrative 002). Zero new findings; F011 (`TryComputeExtension` bug) verified as fixed in place via Phase 2.5 PR #14; F012 (saga loads `SellerId` via `AggregateStreamAsync`) routed `document-as-intentional` in narrative 001 with the lived inline comment preserving the design rationale. W002 confirmed clean against ADR 011 (zero Polecat / SQL Server references). Slice 5.2 (`ReserveMet`) lifted from "P1 / forward-spec per narrative 001" to "shipped lived" via the `PlaceBidHandler.cs:126` lived emission. Closes the lived-BC narrative wave; methodology log Entry 001 written at session close capturing the audit-floor heterogeneity observation across narratives 002-005.
