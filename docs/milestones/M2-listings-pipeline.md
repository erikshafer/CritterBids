...
S1 (Marten ADR — docs only)
 └── S2 (Selling scaffold)
      └── S3 (RegisteredSellers consumer + Program.cs routing)
           └── S4 (CreateDraftListing + ListingValidator)
                └── S5 (SubmitListing → ListingPublished)
                     └── S6 (Listings BC + CatalogListingView + read paths)
                          └── S7 (skills + retro + M2 close)
```
