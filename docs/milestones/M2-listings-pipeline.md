...
| 2 | `docs/prompts/M2-S2-selling-bc-scaffold.md` | Selling BC scaffold — `CritterBids.Selling` and `CritterBids.Selling.Tests` projects, `SellerListing` aggregate (empty shell), `AddSellingModule()` with Marten config + `IntegrateWithWolverine()`, smoke test. No handlers, no slices. |
