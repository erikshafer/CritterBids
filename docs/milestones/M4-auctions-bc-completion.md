...
- **`AttachListingToSession`** — rejects if the listing is not in `Published` status; the exact check mechanism (cross-BC read of `CatalogListingView`, Auctions-side duplicate projection, lightweight Selling signal, or accepted handler-time ambiguity) is M4-D4, pinned in S1. Also rejects if the session is already started.
...
|---|---|---|
| ADR 007 Gate 4 | Event row ID strategy — close with JasperFx input or re-defer with specific new trigger | **Resolve in S1.** This is Gate 4's final scheduled re-evaluation before Auctions BC close. Letting it drift past M4 is not acceptable. |
| M4-D1 | Proxy saga composite key format — `$"{ListingId}:{BidderId}"` vs byte-concatenation before UUID v5 hash | **Resolve in S1.** Workshop 002 §4.1 specifies the string form; S1 confirms it and pins it in the `AuctionsIdentityNamespaces` class (or equivalent). |
| M4-D2 | Session aggregate ID — UUID v7 (new entity, no natural key) or UUID v5 (deterministic from Title + CreatedAt) | **Resolve in S1.** Lean toward UUID v7 — no natural business key exists, and the M3 precedent for all other event-sourced aggregates is v7. UUID v5 would require arguing that Title is a stable business key, which it is not. |
| M4-D3 | ADR 014 timing — author at S6 (second Path A application landed) or bundled into S7 retro | **Call in S6.** Author alongside the code that justifies it, per the rule of thumb that ADRs record lived decisions. S7 reviews and closes. |
| M4-D4 | `AttachListingToSession` published-status check — query `CatalogListingView` directly from Auctions, require Selling to publish a lightweight "listing-available" signal, accept the handler-time ambiguity, or maintain an Auctions-side duplicate `PublishedListings` projection | **Resolve in S1.** Promoted from S5. This choice is architecturally precedent-setting — it is the first time an Auctions handler would read from a Listings-owned view (option 1), or alternatively the first intentional cross-BC duplicate projection (option 4). S5 cannot be sized honestly until the shape is pinned. If S1 resolves to "query `CatalogListingView` directly from Auctions," that decision is cross-cutting enough to warrant its own ADR — flag as **ADR 015 candidate** (Cross-BC read access from handlers) for authorship in S7 or earlier. |
