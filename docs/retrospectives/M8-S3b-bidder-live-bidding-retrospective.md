...
> **Caveat (2026-06-09, added post-merge).** The "all over `BiddingHub`" / end-to-end claim above is the *frontend* truth — the components, optimistic reconcile, cache bridge, and derived beats are all correct and tested. A subsequent **integrated manual run** against the live AppHost (not covered by the mocked Vitest suite) found that the **live cross-client loop does not yet close**: two backend integration bugs sit between the correct frontend and a working live demo. **Bug #1** (Flash listings never reached `Open`) is **fixed** (`fix/flash-open-stream-collision`). **Bug #2** (an HTTP-placed `BidPlaced` does not forward to the Listings read model or the `BiddingHub`) is **open** — so a bid is accepted (200) and shows optimistically, but the `onSettled` re-query reverts it because the read model stays stale. Both are documented in [`docs/notes/integrated-host-flash-bidding-findings.md`](../notes/integrated-host-flash-bidding-findings.md). The M8-S7 multi-context e2e (deferred from this slice) is exactly the test that would have caught this; until Bug #2 lands, treat the live propagation as not-yet-verified.
