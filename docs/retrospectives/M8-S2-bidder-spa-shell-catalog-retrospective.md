...
**5. Session POST needs a JSON body (found by the live smoke check, fixed).** `POST /api/participants/session` binds the `StartParticipantSession` command from the request body; an empty body returns **400 "Invalid JSON format"**. The first `SessionContext` sent no body, so it would have failed against the real API — but the mocked-`fetch` unit tests could not catch it (they never exercise the real request contract). The live smoke check surfaced it immediately; the fix sends `body: "{}"` + `Content-Type: application/json`, verified **`201 Created`** both directly and through the Vite proxy. This is the clearest argument in the slice for live verification: a green build + green unit tests said nothing about a request-shape contract only the running API enforces.
...
**Live smoke check (2026-06-08, against a running Aspire AppHost — Docker up).** AppHost started (`dotnet run --project src/CritterBids.AppHost --launch-profile http`), provisioning Postgres + RabbitMQ + the API at `:5180`; Vite dev at `:5173`. Verified, both directly and **through the Vite dev proxy**: `GET /api/listings` → `200 []` (empty catalog — fresh DB; no HTTP publish endpoint exists to seed one); `POST /api/participants/session` (with the `{}` body fix) → **`201 Created`**, `Location: /api/participants/{id}`, body `{"value":"…","url":"…"}` (**camelCase confirmed** — the same System.Text.Json web defaults the catalog serializer uses, so `CatalogListingView` field casing is confirmed-by-inference); `/hub/bidding/negotiate` → `200` through the `ws:true` proxy. The catalog-with-data render path was exercised only via the jsdom component test (mocked data), since seeding a published listing needs the multi-step register-seller → draft → submit flow with no HTTP publish endpoint. Chromium for the Playwright MCP was unavailable (as at S1), so the browser-render smoke was substituted by the proxy/HTTP integration checks above plus the build + jsdom tests. Verification infra torn down afterward.
...
- **Live smoke check was performed** (see Test results) and caught the session-POST body bug, now fixed. Confirmed live: camelCase JSON keys, the `CreationResponse` `Location` header, and `/hub/bidding` negotiate through the Vite proxy. **Still un-exercised live:** the catalog-with-data render (only the empty state was reachable without a seeded listing) and an actual browser render (Chromium unavailable for the MCP). A future session with a seeded published listing — or the M8-S7 Playwright e2e — should drive the catalog and detail with real data in a real browser.
