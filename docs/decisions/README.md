...
The next unreserved ADR number is **`027-<slug>.md`**. ADR 014 was authored at M5-S6
...
| [026](026-signalr-integration-pattern.md) | SignalR Integration Pattern | ✅ Accepted | 2026-06-08 | Resolves ADR 013's deferred "SignalR integration pattern" question, authored at M8-S3b once the first hub was wired from a client. One app-wide `SignalRProvider` Context owns a single `/hub/bidding` `HubConnection`; components subscribe via a `useListen(handler)` hook; the load-bearing rule is the TanStack Query **cache bridge** — a push is an `invalidateQueries` re-query signal, never a rendered-as-truth payload (milestone §6 / M7 §5). `parseHubMessage` Zod-normalizes Relay's **heterogeneous** push wire (four notification records, no uniform discriminator) into one discriminated union. Live beats (outbid, extended bidding, gavel) are derived from re-queried view transitions, not push payloads — "outbid" is a client derivation because no server `Outbid` push exists. Builds on ADR 023 (wire contract) + the `wolverine-signalr` skill (client conventions). Reusable by M8-S5's ops app against `OperationsHub`. |
| [027](027-per-bc-sticky-queue-bindings.md) | Per-BC Sticky Queue Bindings under Separated Fan-Out | ✅ Accepted | 2026-06-09 | Authored after the M8 Bug #2 root-cause work exposed that the per-BC RabbitMQ queues do not partition consumption: under `MultipleHandlerBehavior.Separated`, Wolverine's fan-out relays every queue's copy to every sticky local handler, so each consumer runs once per consuming queue (`BidPlaced` ×3) — the source of the saga-start dead-letter noise and the recurring client-dedupe obligation. Decision: bind each BC's broker-fed handlers sticky to that BC's own queue (`[StickyHandler]`/`AddStickyHandler`) for exactly-once-per-consumer delivery, plus a new `auctions-auctions-events` queue for Auctions' own dispatcher bridges (sticky matches suppress fan-out). Alternatives (one-queue-per-family + fan-out; documented status quo) rejected for erasing the per-BC teaching topology or keeping untruthful semantics. Implementation scheduled as its own session (M8-S3c prompt); orthogonal to the upstream Wolverine single-saga fix. |
