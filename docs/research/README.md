...
| [`ops-feed-completion-evaluation-comparison.md`](./ops-feed-completion-evaluation-comparison.md) (+ [`evaluation A`](./ops-feed-completion-evaluation.md), [`evaluation B`](./ops-feed-completion-evaluation-es-specialist.md)) | ✅ Decided (→ M8-S6b, milestone v0.5) | Dual-evaluation of the Relay ops-feed completion decision (two independent lenses, compared); §Decision sanctions the pre-S7 slice, the Addendum folds in the dispute-resolution-control decision. |
...
| [`tanstack-db-research.md`](./tanstack-db-research.md) | 📚 Parked — spike candidate (post-M8) | TanStack DB (beta) as client-side projections over the Critter Stack: evaluated against ADR 026's push-=-re-query pattern; three adoption postures, design rules, one-page spike plan, and the "Projections All the Way Down" blog angle. |
