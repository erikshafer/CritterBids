...
- **v0.5** (2026-06-12): M8-S7 — the bid-war spine (Moments 1–7) is now **validated end-to-end by an automated two-bidder Playwright test** (`client/e2e/tests/bid-war.spec.ts`), not only by manual smoke: two isolated browser contexts mint two anonymous sessions (Moment 1), open the seeded listing (Moments 2–3 via the dev seed endpoint), bid $30 / $35 / $55 with live cross-context propagation and the outbid affordance (Moments 4–5), reclaim inside the trigger window with the reserve crossed and the close extended (Moment 6), and observe the gavel fall with the winner rendered correctly in both contexts (Moment 7). One audit note from the automation: Moment 6's claim that Listings "consumes `ExtendedBiddingTriggered` and upserts `ScheduledCloseAt = NewCloseAt`" remains **forward-spec** — the lived `AuctionStatusHandler` carries no `ExtendedBiddingTriggered` handler, so `CatalogListingView.ScheduledCloseAt` never advances and the bidder app's extended-bidding banner cannot fire from the lived read model (the saga-side extension itself is lived and e2e-proven via the listing outliving its original close). Recorded as an M8-S7 carry-forward; lane: `narrative-update` (this row) + `code-update` candidate for a future slice.
