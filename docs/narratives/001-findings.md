...
### Finding 013 - Narrative Moment 3 names a single `SessionMembershipHandler`; M4-S6 split it into two single-source siblings per ADR-014 Sub-Option A

**Routing:** narrative-update

**Surfaced at:** Moment 3 (lived-code audit ran at M4-S7, M4 milestone close)

**Discrepancy.** The narrative's Moment 3 "Response" paragraph at `docs/narratives/001-bidder-wins-flash-auction.md:91` names a single `SessionMembershipHandler` on the Listings side that "consumes each one and tolerantly upserts `CatalogListingView.Status = "Open"`". M4-S6 pinned ADR-014's sub-question to Sub-Option A (one handler class per source BC, single-source per sibling); the lived implementation at M4-S6 close ships two new sibling classes — `src/CritterBids.Listings/AuctionsSessionHandler.cs` (Auctions-sourced, consumes `ListingAttachedToSession` + `SessionStarted` from `listings-auctions-events`) and `src/CritterBids.Listings/SellingListingWithdrawnHandler.cs` (Selling-sourced, consumes `ListingWithdrawn` from `listings-selling-events`) — plus a `Withdrawn`-preservation guard added to the pre-existing `AuctionStatusHandler.Handle(BiddingOpened)`. The narrative's `BiddingOpened` upsert is the lived `AuctionStatusHandler`'s job (carried over from M3-S6), not a `SessionMembershipHandler`'s. The narrative names the *logical feature* (session-membership reflection); the lived *class topology* is the source-split per ADR-014 Sub-Option A. Both expressions are correct in their own register. The Moment 3 `defer` disposition in "Things deliberately not included" explicitly named M4-S5 + M4-S6 as the trigger condition; both have shipped (M4-S5 squash-merged at `b995193`, M4-S6 at `32d6e70`), so the lived-code audit can now run.

**Resolution.** Phase 2.5 follow-up under `docs/prompts/implementations/phase2-5-narrative-001-moment-3-flash-cascade-audit.md` (stubbed at M4-S7, M4 close). Per ADR 016 Phase 2 discipline (separation of audit-and-fix from author-and-ship), the M4-S7 close-out session surfaces the finding and stubs the follow-up; the narrative amendment lands in its own Phase 2.5 PR. Proposed amendment shape: replace the `SessionMembershipHandler` reference with the two-class lived topology + one-line note pointing at ADR-014 Sub-Option A; strike the `defer` line from Moment 3's "Things deliberately not included" and replace with a one-line pointer to this finding's resolution. No `.cs` change — the lived implementation is correct; only the narrative drifts.
