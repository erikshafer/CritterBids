...
- **v1.2** (2026-06-11): M8-S6b lands **Moment 2 — the operator acts.** The dispute card in `client/ops/` now carries the "Resolve with extension" control (rendered only when the card has its `DisputeId`), posting the existing `StaffOnly` `POST /api/obligations/disputes/resolve` with `ResolutionType: "Extension"` — the one non-terminal resolution; `Refund`/`Closed` stay un-surfaced as v1.1 recorded. The control holds a pending state after the 202 and the card clears the queue **live** via the existing `DisputeResolved` push re-query — no manual cache write; the card leaving the data is the success signal — verified end-to-end against the Aspire host (dispute raised, extension granted from the dashboard, card cleared). The v1.1 push-surface gap is also closed: `DeadlineEscalated` now reaches the `OperationsHub` (the M8-S6b Relay ops-feed completion), so escalation *arrivals* land in Morgan's queue live and the polling stopgap is retired. Still deferred: the buyer's "Report a problem" form (`OpenDispute` from a UI).
- **v1.1** (2026-06-10): M8-S6 partial landing of the `UX-or-UI-detail` deferred item. The staff dashboard's **open-dispute queue and escalation queue now render** in `client/ops/` (the M8 frontend this narrative deferred to): the dispute card carries what Moment 1 names — the listing (render-time title join), the raiser, the reason, the opened-at, and the escalation history — and arrives/clears live via the `DisputeOpened`/`DisputeResolved` ops-feed pushes through the ADR 026 cache bridge. Still deferred: the **dispute-resolution controls** (Moment 2's `ResolveDispute` action from the dashboard — unscoped in M8; the M8-S6 retro carries it) and the buyer's "Report a problem" form. One push-surface gap recorded at M8-S6: `DeadlineEscalated` does not reach the `OperationsHub`, so escalation *arrivals* refresh by polling stopgap until a Relay ops-feed completion slice lands.
